...
const [showNewOrderAlert, setShowNewOrderAlert] = useState(false);
  ...
  const daysInRange = useMemo(() => {
    ...
    return res;
  }, [useCustomDateRange, reportStartDate, reportEndDate]);

  useEffect(() => {
    if (useCustomDateRange) setSelectedDay(reportEndDate);
    else setSelectedDay(null);
  }, [useCustomDateRange, reportEndDate]);
            ...
            {useCustomDateRange && (
              <>
                ...
                <div className="flex gap-2 overflow-x-auto mt-2 pb-1 scrollbar-hide">
                  {daysInRange.map((d) => {
                    const sum = entries.filter((e) => e.type === 'income' && e.date === d).reduce((s, e) => s + e.amount, 0);
                    const active = selectedDay ? selectedDay === d : d === reportEndDate;
                    return (
                      <button
                        key={d}
                        onClick={() => setSelectedDay(d)}
                        className={`px-3 py-1.5 rounded-lg text-xs font-bold whitespace-nowrap border ${active ? 'bg-gray-900 text-white border-gray-900' : 'bg-white text-gray-600 border-gray-200'}`}
                      >
                        {d.slice(5).replace('-', '/')} · ${sum}
                      </button>
                    );
                  })}
                </div>
              ...
              entries
                .filter(e => {
                  ...
                  if (useCustomDateRange) {
                    const inRange = e.date >= reportStartDate && e.date <= reportEndDate;
                    const onSelected = selectedDay ? e.date === selectedDay : true;
                    return inRange && onSelected;
                  }
